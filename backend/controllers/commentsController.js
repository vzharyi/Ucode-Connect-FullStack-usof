...
class CommentController {
    ...
    async deleteComment(req, res) {
        try {
            ...
            if (comment.posts && comment.posts.locked && user.role !== 'admin') {
                return res.status(403).json({ error: 'This post is locked and you cannot delete your comment' });
            }
    ...
    async createReply(req, res) {
        try {
            ...
            const post = await Post.findByPk(parentComment.post_id);
            if (post && post.locked) {
                return res.status(403).json({ error: 'This post is locked and cannot be commented on' });
            }
            ...
            await updateCommentRating(reply.id);
            await updateUserRating(reply.author_id);
            ...
            return res.status(500).json({ error: 'Error creating reply', details: error.message });
        }
    }
}
