...
import PostCategories from "../models/posts_categories.js";
    ...
    async getPosts(req, res) {
        try {
            ...
            if (postId) {
                ...
                const { sort = 'likes', categories, startDate, endDate, locked} = req.query;
                ...
                if (categories) {
                    ...
                    const postIdsWithCategory = await PostCategories.findAll({
                        attributes: ['post_id'],
                        where: {category_id: categoryIds},
                    });

                    const postIds = postIdsWithCategory.map((pc) => pc.post_id);
        ...
        try {
            ...
            if (!title || !content) {
                return res.status(400).json({
                    error: 'All fields (title, content, categories) are required, and categories should be an array',
                });
            }
